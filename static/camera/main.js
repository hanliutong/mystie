...
function setState() {
	...
	for(var i=0;i<54;i++) {
		var id = "c_" + i.toString();
		document.getElementById(id).value = state[i];
	...
	W = W.substring(0, col.length - 2);
	Y = Y.substring(0, col.length - 2);
	B = B.substring(0, col.length - 2);
	G = G.substring(0, col.length - 2);
	O = O.substring(0, col.length - 2);
	R = R.substring(0, col.length - 2);
